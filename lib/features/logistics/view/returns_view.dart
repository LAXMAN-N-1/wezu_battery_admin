...
class _ReturnsViewState extends SafeState<ReturnsView> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      setState(() {
        ...
        _returns = results[1] as List<dynamic>;
        _isLoading = false;
      });
    } catch (e) {
      if (!mounted) {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          PageHeader(
            title: 'Returns',
            subtitle: 'Process return requests, inspections, and refunds.',
          ...
          if (!_isLoading) ...[
            Row(
              children: [
                ...
                _stat(
                  'Total Refunds',
                  '₹${NumberFormat('#,##0').format(_stats['total_refund_amount'] ?? 0)}',
                ...
                for (final f in [null, ..._pipeline, 'cancelled'])
                  Padding(
                    ...
                    child: FilterChip(
                      ...
                      label: Text(
                        (f ?? 'ALL').replaceAll('_', ' ').toUpperCase(),
                        style: TextStyle(
                        ...
                        return [
                          ...
                          Text(
                            r['refund_amount'] != null
                                ? '₹${NumberFormat('#,##0').format(r['refund_amount'])}'
                                : '—',
                            style: TextStyle(
                              color: r['refund_amount'] != null
                                  ? const Color(0xFF22C55E)
                                  : Colors.white30,
  ...
  Widget _statusPipeline(String status) {
    ...
    return Row(
      ...
      children: List.generate(_pipeline.length, (i) {
  ...
  Future<void> _updateStatus(int id, String status) async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(ok ? 'Updated to $status' : 'Failed'),
  ...
  void _showDetail(Map<String, dynamic> r) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(
          children: [
            const Icon(Icons.assignment_return, color: Color(0xFFF59E0B)),
            const SizedBox(width: 8),
            Text(
              'Return #${r['id']}',
              style: GoogleFonts.outfit(
          ...
          child: Column(
            ...
            children: [
              ...
              _row(
                'Refund Amount',
                r['refund_amount'] != null
                    ? '₹${NumberFormat('#,##0.00').format(r['refund_amount'])}'
                    : 'Not set',
              ),
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              _errorMessage ?? 'Unable to load returns.',
