...
class _RentalHistoryViewState extends State<RentalHistoryView> {
  ...
  void _applyFilters() {
    _filtered = _history.where((r) {
      if (_statusFilter != 'all' && r.status.toLowerCase() != _statusFilter.toLowerCase()) return false;
      return true;
    }).toList();
  }
  ...
  Widget build(BuildContext context) {
    final completed = _history.where((r) => r.status.toUpperCase() == 'COMPLETED').length;
    final cancelled = _history.where((r) => r.status.toUpperCase() == 'CANCELLED').length;
    final overdue = _history.where((r) => r.status.toUpperCase() == 'OVERDUE').length;
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    _buildStat('Total Records', '${_history.length}', Icons.history_outlined, const Color(0xFF3B82F6)),
                    const SizedBox(width: 12),
                    _buildStat('Completed', '$completed', Icons.check_circle_outline, const Color(0xFF22C55E)),
                    const SizedBox(width: 12),
                    _buildStat('Overdue', '$overdue', Icons.warning_amber_outlined, const Color(0xFFF59E0B)),
                    const SizedBox(width: 12),
                    _buildStat('Revenue', '₹${NumberFormat('#,##0').format(totalRev)}', Icons.currency_rupee_outlined, const Color(0xFF8B5CF6)),
                  ],
                ...
                Row(
                  children: [
                    ...
                    const SizedBox(width: 12),
                    ...['all', 'COMPLETED', 'OVERDUE', 'CANCELLED'].map((s) =>
                      Padding(
                        padding: const EdgeInsets.only(left: 6),
                        child: FilterChip(
                          selected: _statusFilter == s,
                          label: Text(s == 'all' ? 'All' : s[0] + s.substring(1).toLowerCase(), style: TextStyle(color: _statusFilter == s ? Colors.white : Colors.white54, fontSize: 12)),
                  ...
                  child: AdvancedCard(
                    ...
                    child: _isLoading
                      ...
                      : _filtered.isEmpty
                        ...
                        : AdvancedTable(
                            columns: const ['ID', 'User', 'Battery', 'Start', 'End', 'Amount', 'Status'],
                            rows: _filtered.map((r) => [
                              ...
                              Text('BAT_${r.batteryId}', style: const TextStyle(color: Colors.white70, fontSize: 12)),
                              Text(DateFormat('MMM dd HH:mm').format(r.startTime), style: const TextStyle(color: Colors.white54, fontSize: 12)),
                              Text(r.endTime != null ? DateFormat('MMM dd HH:mm').format(r.endTime!) : '—', style: const TextStyle(color: Colors.white54, fontSize: 12)),
                              Text('₹${r.totalAmount.toStringAsFixed(0)}', style: const TextStyle(color: Color(0xFF22C55E), fontWeight: FontWeight.bold)),
                              StatusBadge(status: r.status),
                            ]).toList(),
                            onRowTap: (i) => setState(() => _selectedRental = _filtered[i]),
                          ),
                  ),
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text('Rental #${r.id}', style: GoogleFonts.outfit(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.white)),
              IconButton(icon: const Icon(Icons.close, color: Colors.white54), onPressed: () => setState(() => _selectedRental = null)),
            ],
          ),
          ...
          _infoRow('Start', DateFormat('MMM dd, yyyy HH:mm').format(r.startTime)),
          _infoRow('Expected End', DateFormat('MMM dd, yyyy HH:mm').format(r.expectedEndTime)),
          if (r.endTime != null) _infoRow('Actual End', DateFormat('MMM dd, yyyy HH:mm').format(r.endTime!)),
          ...
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('BATTERY LEVEL', style: GoogleFonts.inter(fontSize: 10, color: Colors.white38, letterSpacing: 1.5)),
                ...
                ClipRRect(
                  ...
                  child: LinearProgressIndicator(
                    ...
                    valueColor: AlwaysStoppedAnimation(
                      (r.batteryLevel ?? 0) > 60 ? const Color(0xFF22C55E) : (r.batteryLevel ?? 0) > 30 ? const Color(0xFFF59E0B) : const Color(0xFFEF4444),
                    ),
                    minHeight: 8,
                  ),
                ),
