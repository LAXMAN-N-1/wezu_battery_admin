          ...
          if (!_isLoading) ...[
            Row(
              children: [
                ...
                _stat(
                  'Total Amount',
                  '₹${NumberFormat('#,##0').format(_stats['total_amount'] ?? 0)}',
                  Icons.payments,
                ...
                _stat(
                  'Tax Collected',
                  '₹${NumberFormat('#,##0').format(_stats['total_tax_collected'] ?? 0)}',
                    ...
                    child: AdvancedTable(
                      ...
                      rows: _invoices.map((inv) {
                        ...
                        return [
                          ...
                          Text(
                            '₹${NumberFormat('#,##0.00').format(inv['subtotal'] ?? 0)}',
                            style: const TextStyle(color: Colors.white70),
                          ),
                          Text(
                            '₹${NumberFormat('#,##0.00').format(inv['tax_amount'] ?? 0)}',
                            style: const TextStyle(
                          ...
                          Text(
                            '₹${NumberFormat('#,##0.00').format(inv['total'] ?? 0)}',
  ...
  void _showDetail(Map<String, dynamic> inv) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(
          children: [
            const Icon(Icons.receipt, color: Color(0xFF3B82F6)),
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              _row(
                'Late Fee Invoice',
                inv['is_late_fee'] == true ? 'Yes' : 'No',
              ),
              _row(
                'Date',
                DateFormat('MMMM d, yyyy h:mm a').format(
                  DateTime.tryParse(inv['created_at'] ?? '') ?? DateTime.now(),
                ),
              ),
            ],
          ),
