...
class _TransactionsViewState extends State<TransactionsView> {
  ...
  Future<void> _loadData() async {
    ...
    final (stats, data) = await ParallelLoad.pair(
      _repo.getTransactionStats(),
      _repo.getTransactions(type: _typeFilter, status: _statusFilter),
    );
    if (!mounted) return;
    setState(() {
      _stats = stats;
          ...
          if (!_isLoading) ...[
            ...
            Row(
              children: [
                ...
                _stat(
                  "Today's Volume",
                  '₹${NumberFormat('#,##0').format(_stats['today_amount'] ?? 0)}',
                  Icons.today,
                    ...
                    child: AdvancedTable(
                      ...
                      rows: _transactions.map((tx) {
                        ...
                        return [
                          ...
                          Text(
                            '₹${NumberFormat('#,##0.00').format(tx['amount'] ?? 0)}',
                          ...
                          Text(
                            '₹${NumberFormat('#,##0').format(tx['tax_amount'] ?? 0)}',
                            style: const TextStyle(
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              _detailRow(
                'Tax',
                '₹${NumberFormat('#,##0.00').format(tx['tax_amount'] ?? 0)}',
              ),
              _detailRow(
                'Payment Method',
