...
class _EnergyMonitoringViewState extends SafeState<EnergyMonitoringView> {
        ...
        children: [
          PageHeader(
            ...
            actionButton: Row(
              ...
              children: [
                _buildLabeledFilter('BESS Unit', _buildUnitFilter()),
                const SizedBox(width: 12),
                _buildLabeledFilter('Time Range', _buildTimeFilter()),
              ],
            ),
  ...
  Widget _buildLabeledFilter(String label, Widget child) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(
            ...
            child: Row(
              crossAxisAlignment: CrossAxisAlignment.end,
              children: _summary.map((day) {
                final charged = (day['charged'] as num?)?.toDouble() ?? 0;
                final discharged = (day['discharged'] as num?)?.toDouble() ?? 0;
                final maxVal = _summary.fold<double>(
                  0,
                  (m, d) => [
                    m,
                    (d['charged'] as num?)?.toDouble() ?? 0,
                    (d['discharged'] as num?)?.toDouble() ?? 0,
                  ].reduce((a, b) => a > b ? a : b),
                );
                return Expanded(
                  child: Padding(
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            Container(
                              width: 14,
                              height: maxVal > 0 ? (charged / maxVal * 120) : 0,
                              decoration: BoxDecoration(
                                color: const Color(
                                  0xFF22C55E,
        ...
        children: [
          Padding(
            ...
            child: Row(
              children: [
                Text(
                  'Energy Logs (${_logs.length} records)',
                  style: GoogleFonts.outfit(
                    fontSize: 16,
                    fontWeight: FontWeight.w600,
                    color: Colors.white,
                    ...
                    return [
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          const SizedBox(width: 4),
                          Text(
                            log.powerKw.abs().toStringAsFixed(1),
                            style: TextStyle(
                              color: isCharging
                                  ? const Color(0xFF22C55E)
                                  : const Color(0xFFF59E0B),
                      ...
                      Text(
                        log.energyKwh.toStringAsFixed(2),
                        style: const TextStyle(color: Colors.white),
                      ),
                      Text(
                        '${log.socStart.toStringAsFixed(1)}%',
                        style: const TextStyle(color: Colors.white54),
                      ),
                      Text(
                        '${log.socEnd.toStringAsFixed(1)}%',
                        style: const TextStyle(color: Colors.white54),
                      ),
  ...
  String _friendlyError(Object error) {
    ...
    return 'Unable to load energy monitoring data.';
  }
}
