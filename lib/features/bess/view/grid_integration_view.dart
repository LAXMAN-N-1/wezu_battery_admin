...
class _GridIntegrationViewState extends SafeState<GridIntegrationView> {
    ...
    try {
      ...
      setState(() {
        ...
        if (_filterStatus != null) {
          _events = _events.where((e) => e.status == _filterStatus).toList();
        }
        _isLoading = false;
      });
    } catch (e) {
        ...
        children: [
          ...
          Row(
            children: [
              ...
              _filterChip('Peak Shaving', _filterType == 'peak_shaving', () {
                setState(() => _filterType = 'peak_shaving');
                _loadData();
              }),
              const SizedBox(width: 8),
              _filterChip('Load Shifting', _filterType == 'load_shifting', () {
                setState(() => _filterType = 'load_shifting');
                _loadData();
              }),
              const SizedBox(width: 8),
              _filterChip(
                'Freq. Regulation',
                _filterType == 'frequency_regulation',
                () {
                  setState(() => _filterType = 'frequency_regulation');
                  _loadData();
                },
              ),
              const SizedBox(width: 8),
              _filterChip('Backup', _filterType == 'backup', () {
                setState(() => _filterType = 'backup');
                _loadData();
              }),
              const Spacer(),
              _statusFilter(),
  ...
  Widget _buildSummaryCards() {
    ...
    final totalRevenue = completed.fold<double>(
      0,
      (sum, e) => sum + (e.revenueEarned ?? 0),
    );
    final totalEnergy = completed.fold<double>(
      0,
      (sum, e) => sum + (e.energyKwh ?? 0),
    );
          ...
          : AdvancedTable(
              ...
              rows: _events.map((e) {
                ...
                return [
                  ...
                  Text(
                    _formatTs(e.startTime),
                    style: const TextStyle(color: Colors.white54, fontSize: 12),
                  ),
                  Text(
                    e.actualPowerKw?.toStringAsFixed(1) ??
                        e.targetPowerKw.toStringAsFixed(1),
                    style: const TextStyle(color: Colors.white),
                  ),
                  Text(
                    e.energyKwh?.toStringAsFixed(1) ?? '—',
                    style: const TextStyle(color: Colors.white54),
                  ),
                  Text(
                    e.revenueEarned != null
                        ? '₹${e.revenueEarned!.toStringAsFixed(0)}'
                        : '—',
                    style: TextStyle(
                      color: e.revenueEarned != null
                          ? const Color(0xFF22C55E)
                          : Colors.white38,
                    ),
                  ),
  ...
  void _showCreateDialog() {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Grid event creation coming soon.',
          style: TextStyle(color: Colors.white54),
        ),
        actions: [
          TextButton(
