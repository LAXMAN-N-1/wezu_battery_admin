    ...
    return SingleChildScrollView(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text('Manage frequently asked questions', style: TextStyle(color: Colors.white54, fontSize: 14)),
        ])),
      ...
      Row(children: ['All', 'general', 'payment', 'rental'].map((cat) {
        ...
        return Padding(padding: const EdgeInsets.only(right: 8), child: GestureDetector(
          ...
          child: Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
            ...
            child: Text(cat == 'All' ? 'All' : cat.toUpperCase(), style: TextStyle(color: selected ? const Color(0xFF3B82F6) : Colors.white54, fontSize: 12)))));
      }).toList()),
      const SizedBox(height: 16),
    ...
    return Container(margin: const EdgeInsets.only(bottom: 10), decoration: BoxDecoration(
      ...
      child: ExpansionTile(
        ...
        leading: CircleAvatar(radius: 14, backgroundColor: Colors.blue.withValues(alpha: 0.1),
          child: Text('${index + 1}', style: TextStyle(color: Colors.blue, fontSize: 12, fontWeight: FontWeight.bold))),
        ...
        subtitle: Row(children: [
          Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), margin: const EdgeInsets.only(top: 4),
            ...
            child: Text(faq['category']?.toString().toUpperCase() ?? '', style: TextStyle(color: Colors.white38, fontSize: 9))),
        ...
        children: [
          Text(faq['answer']?.toString() ?? '', style: TextStyle(color: Colors.white70, fontSize: 13, height: 1.6)),
          const SizedBox(height: 12),
          Row(children: [
            Icon(Icons.thumb_up, size: 14, color: Colors.green.withValues(alpha: 0.6)), const SizedBox(width: 4),
            Text('$helpful', style: TextStyle(color: Colors.green.withValues(alpha: 0.6), fontSize: 12)),
            const SizedBox(width: 12),
            Icon(Icons.thumb_down, size: 14, color: Colors.red.withValues(alpha: 0.6)), const SizedBox(width: 4),
            Text('$notHelpful', style: TextStyle(color: Colors.red.withValues(alpha: 0.6), fontSize: 12)),
            const Spacer(),
