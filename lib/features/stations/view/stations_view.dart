...
class _StationsViewState extends ConsumerState<StationsView> {
  ...
  Widget _buildStationStatusBadge(Station station) {
    ...
    } else if (station.status == 'inactive' || station.status == 'closed') {
      label = 'INACTIVE';
      color = Colors.grey;
      icon = Icons.pause_circle_filled;
    } else {
      final hours = StationHours.fromJsonString(station.openingHours);
      final isPageOpen = hours.isOpen(DateTime.now(), is24x7: station.is24x7);
      label = isPageOpen ? 'OPEN' : 'CLOSED';
