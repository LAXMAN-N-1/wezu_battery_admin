...
class _TeamPerformanceViewState extends State<TeamPerformanceView> {
  ...
  Widget build(BuildContext context) {
    ...
    final agents = (_perfData['agents'] as List?)?.cast<AgentPerformance>() ?? [];
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            flex: 1,
            child: _buildAgentLeaderboard(agents),
          ),
        ],
      ),
    );
  ...
  Widget _buildHeader() {
    return PageHeader(
      ...
      subtitle: 'Monitor agent SLAs, CSAT scores, and support desk volume trends.',
      actionButton: ElevatedButton.icon(
        onPressed: () => _loadData(),
        icon: const Icon(Icons.refresh, size: 20, color: Colors.white),
        label: const Text('Refresh Data', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
  ...
  Widget _buildSlaCard({required String title, required String value, required IconData icon, required Color color, required String subtitle}) {
    return AdvancedCard(
        ...
        children: [
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  const Text('Created', style: TextStyle(color: Colors.white70, fontSize: 12)),
                  const SizedBox(width: 16),
          ...
          Expanded(
            child: _trends.isEmpty 
              ? const Center(child: Text('No trend data available.', style: TextStyle(color: Colors.white54)))
              : BarChart(
                  BarChartData(
                    alignment: BarChartAlignment.spaceAround,
                    maxY: (_trends.map((t) => t.created > t.resolved ? t.created : t.resolved).reduce((a, b) => a > b ? a : b).toDouble() * 1.5).clamp(10, 1000).toDouble(),
                      ...
                      bottomTitles: AxisTitles(
                        sideTitles: SideTitles(
                          ...
                          getTitlesWidget: (value, meta) {
                            if (value < 0 || value >= _trends.length) return const SizedBox.shrink();
                            return Padding(
                              padding: const EdgeInsets.only(top: 10.0),
                              child: Text(_trends[value.toInt()].date, style: const TextStyle(color: Colors.white54, fontSize: 10)),
                            );
                          },
                          reservedSize: 30,
                        ),
                    ...
                    barGroups: _trends.asMap().entries.map((e) {
                      return BarChartGroupData(
                        x: e.key,
                        barRods: [
                          BarChartRodData(toY: e.value.created.toDouble(), color: const Color(0xFF3B82F6), width: 12, borderRadius: const BorderRadius.vertical(top: Radius.circular(4))),
                          BarChartRodData(toY: e.value.resolved.toDouble(), color: Colors.green, width: 12, borderRadius: const BorderRadius.vertical(top: Radius.circular(4))),
                        ],
                      );
                    }).toList(),
                  ),
        ...
        children: [
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              Text('Agent Leaderboard', style: GoogleFonts.inter(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
            ],
          ),
          const SizedBox(height: 24),
          if (agents.isEmpty)
            const Expanded(child: Center(child: Text('No active agents.', style: TextStyle(color: Colors.white54))))
          else
            Expanded(
  ...
  Widget _buildAgentCard(AgentPerformance agent, int rank) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: rank <= 3 ? rankColor.withValues(alpha: 0.3) : Colors.white12),
      ),
      child: Column(
        children: [
          Row(
            children: [
              ...
              CircleAvatar(
                ...
                child: Text(agent.agentName[0].toUpperCase(), style: const TextStyle(color: Colors.white70)),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Text(agent.agentName, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 15)),
              ),
              Container(
                ...
                child: Row(
                  children: [
                    const Icon(Icons.star, color: Colors.purpleAccent, size: 12),
                    const SizedBox(width: 4),
                    Text(agent.csatScore.toStringAsFixed(1), style: const TextStyle(color: Colors.purpleAccent, fontWeight: FontWeight.bold, fontSize: 12)),
                  ],
                ),
              ),
            ],
          ...
          Row(
            ...
            children: [
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('Resolved', style: TextStyle(color: Colors.white54, fontSize: 11)),
                  Text('${agent.resolved} / ${agent.totalAssigned}', style: const TextStyle(color: Colors.green, fontSize: 14, fontWeight: FontWeight.bold)),
                ],
              ),
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('Avg Resol. Time', style: TextStyle(color: Colors.white54, fontSize: 11)),
                  Text('${agent.avgResolutionHours}h', style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.bold)),
                ],
              ),
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('Resolution Rate', style: TextStyle(color: Colors.white54, fontSize: 11)),
                  Text('${agent.resolutionRate}%', style: const TextStyle(color: Colors.blueAccent, fontSize: 14, fontWeight: FontWeight.bold)),
                ],
              ),
            ],
          ),
