...
class _BatteriesViewState extends ConsumerState<BatteriesView> with TickerProviderStateMixin {
  late final InventoryRepository _repository;
  List<Battery> _batteries = [];
  Map<String, dynamic> _summary = {
    "total_batteries": 0, "available_count": 0, "rented_count": 0,
    "maintenance_count": 0, "retired_count": 0, "utilization_percentage": 0.0
  };
  ...
  bool get _allSelected => _batteries.isNotEmpty && _selectedIds.length == _batteries.length;
  ...
  void initState() {
    ...
    _bulkBarController = AnimationController(vsync: this, duration: const Duration(milliseconds: 300));
    _loadData();
  }
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (_quickFilter == 'Needs Attention') { maxHealth = 60; effectiveStatus = 'All'; }
  ...
  void _onSearchChanged(String query) {
    ...
    _debounce = Timer(const Duration(milliseconds: 300), () {
      setState(() { _searchQuery = query; _currentPage = 0; });
      _loadData();
    });
  }
  ...
  void _toggleSelection(String id) {
    setState(() {
      ...
      if (_selectedIds.isNotEmpty) { _bulkBarController.forward(); }
      else { _bulkBarController.reverse(); }
    });
  }
  ...
  void _clearSelection() {
    setState(() { _selectedIds.clear(); _bulkBarController.reverse(); });
  }
  ...
  void _openAddDrawer() {
    showGeneralDialog(
      ...
      transitionBuilder: (context, a1, a2, child) =>
        SlideTransition(position: Tween(begin: const Offset(1, 0), end: Offset.zero).animate(a1), child: child),
    );
  }
  ...
  void _viewQR(Battery battery) {
    showDialog(
      context: context,
      builder: (context) => BatteryQRModal(serialNumber: battery.serialNumber, qrData: 'WEZU_BAT_${battery.serialNumber}'),
    );
  }
  ...
  Future<void> _exportInventory() async {
    try {
      await _repository.exportBatteries(
        status: _statusFilter, locationType: _locationFilter, batteryType: _batteryTypeFilter);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Export download started'), backgroundColor: Color(0xFF3B82F6)),
        );
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _bulkChangeStatus(String status) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text('Change ${_selectedIds.length} batteries to $status?', style: const TextStyle(color: Colors.white)),
        content: Text('This action will be logged in the audit trail.', style: TextStyle(color: Colors.white.withValues(alpha: 0.6))),
        actions: [
    ...
    if (confirm == true) {
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('${_selectedIds.length} batteries updated to $status'), backgroundColor: Colors.greenAccent.shade700),
          );
        }
      } catch (e) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Are you sure you want to retire ${battery.serialNumber}?', style: const TextStyle(color: Colors.white70)),
          const SizedBox(height: 12),
          const Text('A reason is required for audit tracking.', style: TextStyle(color: Colors.white38, fontSize: 13)),
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                await _repository.deleteBattery(battery.id, reason: reasonController.text);
            ...
            child: const Text('Confirm Retirement', style: TextStyle(color: Colors.white)),
          ),
        ],
      ),
    );
    ...
    return PageHeader(
      ...
      actionButton: Row(
        children: [
          ...
          _outlinedBtn(Icons.upload_file_rounded, 'Bulk Import', () => showDialog(context: context, builder: (_) => const BatteryImportModal())),
          const SizedBox(width: 10),
          Container(
  ...
  Widget _buildStatsRow() {
    ...
    final pctMaint = total > 0 ? '${(maintenance / total * 100).toStringAsFixed(0)}% of fleet' : '—';
    final maintWarn = total > 0 && (maintenance / total * 100) > 20;

    return Row(
      children: [
        Expanded(child: _premiumStatCard('Fleet Total', '$total', Icons.analytics_outlined, const Color(0xFF3B82F6), null, 'All')),
        const SizedBox(width: 14),
        Expanded(child: _premiumStatCard('Ready/Available', '$available', Icons.check_circle_outline_rounded, const Color(0xFF22C55E), pctAvail, 'Available')),
        const SizedBox(width: 14),
        Expanded(child: _premiumStatCard('On Rent', '$rented', Icons.electric_bike_rounded, const Color(0xFF8B5CF6), pctRent, 'Rented')),
        const SizedBox(width: 14),
  ...
  Widget _premiumStatCard(String title, String value, IconData icon, Color color, String? subtitle, String? filterStatus, {bool warning = false}) {
    final isActive = filterStatus != null && _statusFilter == filterStatus;
    return GestureDetector(
      onTap: filterStatus != null ? () => _applyStatCardFilter(filterStatus) : null,
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(color: isActive ? color.withValues(alpha: 0.15) : Colors.black.withValues(alpha: 0.2), blurRadius: 15, offset: const Offset(0, 8)),
          ],
        ),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
              ...
              children: [
                ...
                if (warning)
                  Container(
                    ...
                    child: const Text('⚠ HIGH', style: TextStyle(color: Colors.redAccent, fontSize: 9, fontWeight: FontWeight.bold)),
                  )
                else
            ...
            TweenAnimationBuilder<int>(
              tween: IntTween(begin: 0, end: int.tryParse(value.replaceAll('%', '').replaceAll('.', '')) ?? 0),
              duration: const Duration(milliseconds: 900),
              builder: (context, val, _) {
                final display = value.contains('%') ? '${(val / (value.contains('.') ? 100 : 1)).toStringAsFixed(value.contains('.') ? 2 : 0)}%' : '$val';
                return Text(display, style: GoogleFonts.inter(fontSize: 28, fontWeight: FontWeight.w800, color: Colors.white));
              },
            ),
  ...
  Widget _buildQuickFilters() {
    final filters = ['All', 'Needs Attention', 'Expiring Warranty', 'Idle >7 Days', 'Recently Added'];
    return SizedBox(
      height: 36,
      ...
      children: [
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            suffixIcon: _searchQuery.isNotEmpty
                ? IconButton(icon: const Icon(Icons.close, color: Colors.white38, size: 18), onPressed: () {
                    _searchController.clear();
            ...
            border: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: BorderSide(color: Colors.white.withValues(alpha: 0.06))),
        ...
        Row(
          children: [
            ...
            const SizedBox(width: 10),
            _filterDropdown('Location', _locationFilter, ['All', 'Station', 'Warehouse', 'Service_center', 'Recycling'], (v) {
              setState(() { _locationFilter = v!; _currentPage = 0; });
              _loadData();
            }),
            const SizedBox(width: 10),
            _filterDropdown('Type', _batteryTypeFilter, ['All', '48V/30Ah', '60V/40Ah', '72V/50Ah'], (v) {
              setState(() { _batteryTypeFilter = v!; _currentPage = 0; });
              _loadData();
            }),
            const SizedBox(width: 10),
            _filterDropdown('Health', _healthFilter, ['All', 'Good >80%', 'Fair 50-80%', 'Poor <50%'], (v) {
            ...
            if (_activeFilterCount > 0) ...[
              Container(
                ...
                decoration: BoxDecoration(color: const Color(0xFF3B82F6).withValues(alpha: 0.12), borderRadius: BorderRadius.circular(20)),
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Text('$_activeFilterCount active', style: const TextStyle(color: Color(0xFF3B82F6), fontSize: 12, fontWeight: FontWeight.w600)),
  ...
  Widget _filterDropdown(String label, String value, List<String> options, ValueChanged<String?> onChanged) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: value != 'All' ? const Color(0xFF3B82F6).withValues(alpha: 0.06) : Colors.white.withValues(alpha: 0.04),
        borderRadius: BorderRadius.circular(12),
        border: Border.all(color: value != 'All' ? const Color(0xFF3B82F6).withValues(alpha: 0.2) : Colors.white.withValues(alpha: 0.05)),
      ),
              ...
              : SingleChildScrollView(
                  ...
                  child: DataTable(
                    ...
                    columns: [
                      ...
                      const DataColumn(label: Text('Battery Serial', style: TextStyle(color: Colors.white54, fontWeight: FontWeight.bold, fontSize: 12))),
                      const DataColumn(label: Text('Status', style: TextStyle(color: Colors.white54, fontWeight: FontWeight.bold, fontSize: 12))),
                      const DataColumn(label: Text('Location', style: TextStyle(color: Colors.white54, fontWeight: FontWeight.bold, fontSize: 12))),
                      const DataColumn(label: Text('Type', style: TextStyle(color: Colors.white54, fontWeight: FontWeight.bold, fontSize: 12))),
                      const DataColumn(label: Text('Health', style: TextStyle(color: Colors.white54, fontWeight: FontWeight.bold, fontSize: 12))),
                      const DataColumn(label: Text('Cycles', style: TextStyle(color: Colors.white54, fontWeight: FontWeight.bold, fontSize: 12))),
                      const DataColumn(label: Text('Last Updated', style: TextStyle(color: Colors.white54, fontWeight: FontWeight.bold, fontSize: 12))),
                      const DataColumn(label: Text('Warranty', style: TextStyle(color: Colors.white54, fontWeight: FontWeight.bold, fontSize: 12))),
                      const DataColumn(label: Text('Actions', style: TextStyle(color: Colors.white54, fontWeight: FontWeight.bold, fontSize: 12))),
                    ],
                    rows: _batteries.map((b) {
                      ...
                      return DataRow(
                        ...
                        color: WidgetStateProperty.resolveWith((states) {
                          if (states.contains(WidgetState.selected)) return const Color(0xFF3B82F6).withValues(alpha: 0.06);
                        ...
                        cells: [
                          ...
                          DataCell(Text('${b.cycleCount}', style: const TextStyle(color: Colors.white70, fontSize: 13))),
    ...
    return SizedBox(
      ...
      child: Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          const Icon(Icons.battery_alert_rounded, size: 64, color: Colors.white10),
          const SizedBox(height: 16),
          const Text('No Batteries Matching Criteria', style: TextStyle(color: Colors.white38, fontSize: 16)),
          ...
          Row(mainAxisSize: MainAxisSize.min, children: [
            GestureDetector(
              ...
              child: Container(
                ...
                child: const Icon(Icons.qr_code_2_rounded, size: 13, color: Colors.blueAccent),
              ),
            ),
            const SizedBox(width: 8),
            Text(b.serialNumber, style: GoogleFonts.firaCode(fontWeight: FontWeight.w600, color: Colors.blueAccent, fontSize: 13)),
          ]),
          if (b.manufacturer != null)
            Padding(
              padding: const EdgeInsets.only(left: 24, top: 2),
              child: Text(b.manufacturer!, style: const TextStyle(color: Colors.white30, fontSize: 11)),
            ),
        ]),
      ),
    );
  ...
  Widget _buildStatusCell(Battery b) {
    final statusColors = {
      'available': const Color(0xFF22C55E), 'rented': const Color(0xFF3B82F6),
      'maintenance': const Color(0xFFF59E0B), 'retired': const Color(0xFFEF4444), 'charging': const Color(0xFF8B5CF6),
    };
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        Container(
          ...
          decoration: BoxDecoration(shape: BoxShape.circle, color: color,
            boxShadow: isRented ? [BoxShadow(color: color.withValues(alpha: 0.5), blurRadius: 6)] : null,
          ),
        ),
        const SizedBox(width: 6),
        Text(b.status.toUpperCase(), style: TextStyle(color: color, fontSize: 11, fontWeight: FontWeight.w700, letterSpacing: 0.5)),
      ]),
    );
  }
  ...
  Widget _buildLocationCell(Battery b) {
    final icons = {
      'station': Icons.ev_station_rounded, 'warehouse': Icons.warehouse_rounded,
      'service_center': Icons.build_rounded, 'recycling': Icons.recycling_rounded,
    };
    return Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
      Row(mainAxisSize: MainAxisSize.min, children: [
        Icon(icons[b.locationType.toLowerCase()] ?? Icons.location_on, size: 14, color: Colors.white38),
        const SizedBox(width: 4),
        Text(b.locationType.replaceAll('_', ' ').toUpperCase(), style: const TextStyle(color: Colors.white70, fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 0.3)),
      ]),
      if (b.locationName != null)
        Padding(padding: const EdgeInsets.only(left: 18, top: 1),
          child: Text(b.locationName!, style: const TextStyle(color: Colors.white30, fontSize: 11))),
    ]);
  }
  ...
  Widget _buildTypeCell(Battery b) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
      ...
      Text('Li-ion', style: TextStyle(color: Colors.white.withValues(alpha: 0.25), fontSize: 11)),
    ]);
  }

  Widget _buildHealthCell(double health) {
    Color color = health > 80 ? const Color(0xFF22C55E) : (health > 50 ? const Color(0xFFF59E0B) : const Color(0xFFEF4444));
    return Tooltip(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(width: 8),
        Text('${health.toInt()}%', style: TextStyle(color: color, fontSize: 13, fontWeight: FontWeight.bold)),
      ]),
    );
  }
  ...
  Widget _buildLastUpdatedCell(Battery b) {
    ...
    else if (diff.inDays < 30) { relative = '${diff.inDays}d ago'; }
    else { relative = DateFormat('MMM d').format(b.updatedAt); }

    return Tooltip(
      message: DateFormat('yyyy-MM-dd HH:mm:ss').format(b.updatedAt),
      child: Text(relative, style: TextStyle(color: Colors.white.withValues(alpha: 0.5), fontSize: 13)),
    );
  }

  Widget _buildWarrantyCell(Battery b) {
    if (b.warrantyExpiry == null) {
      return Text('—', style: TextStyle(color: Colors.white.withValues(alpha: 0.2), fontSize: 13));
    }
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: (isValid ? (isExpiringSoon ? Colors.amber : Colors.green) : Colors.red).withValues(alpha: 0.1),
        borderRadius: BorderRadius.circular(12),
      ),
      child: Text(
        isValid ? (isExpiringSoon ? '${daysLeft}d left' : 'Valid') : 'Expired',
        style: TextStyle(
          color: isValid ? (isExpiringSoon ? Colors.amber : Colors.greenAccent) : Colors.redAccent,
  ...
  Widget _buildActionsCell(Battery b) {
    return Row(mainAxisSize: MainAxisSize.min, children: [
      _actionIcon(Icons.info_outline_rounded, Colors.white54, 'Detail', () => _openDetailDrawer(b)),
      _actionIcon(Icons.edit_note_rounded, Colors.blueAccent.withValues(alpha: 0.7), 'Edit', () => _openEditDrawer(b)),
      _actionIcon(Icons.block_rounded, Colors.redAccent.withValues(alpha: 0.7), 'Retire', () => _confirmDelete(b)),
      PopupMenuButton<String>(
        icon: const Icon(Icons.more_vert, size: 18, color: Colors.white38),
        color: const Color(0xFF1E293B),
        onSelected: (val) {
          if (val == 'qr') _viewQR(b);
          if (val == 'copy') { Clipboard.setData(ClipboardData(text: b.serialNumber)); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Serial copied'))); }
        },
        itemBuilder: (_) => [
          const PopupMenuItem(value: 'qr', child: Row(children: [Icon(Icons.qr_code, size: 16, color: Colors.white54), SizedBox(width: 8), Text('QR Code', style: TextStyle(color: Colors.white70))])),
          const PopupMenuItem(value: 'copy', child: Row(children: [Icon(Icons.copy, size: 16, color: Colors.white54), SizedBox(width: 8), Text('Copy Serial', style: TextStyle(color: Colors.white70))])),
        ],
      ),
    ]);
  }
      ...
      builder: (context, child) {
        ...
        return Positioned(
          ...
          child: SlideTransition(
            position: Tween(begin: const Offset(0, 1.5), end: Offset.zero).animate(
              CurvedAnimation(parent: _bulkBarController, curve: Curves.easeOutCubic),
            ),
            child: Container(
              ...
              child: Row(children: [
                ...
                OutlinedButton.icon(
                  ...
                  label: const Text('Export', style: TextStyle(color: Colors.white54, fontSize: 12)),
                  style: OutlinedButton.styleFrom(
                    side: BorderSide(color: Colors.white.withValues(alpha: 0.1)),
        ...
        children: [
          ...
          ...List.generate(totalPages.clamp(0, 7), (i) {
            ...
            return Padding(
              ...
              child: InkWell(
                onTap: () { setState(() => _currentPage = pageIndex); _loadData(); },
                borderRadius: BorderRadius.circular(8),
                child: Container(
                  width: 32, height: 32,
                  alignment: Alignment.center,
                  decoration: BoxDecoration(
                    color: isCurrentPage ? const Color(0xFF3B82F6) : Colors.transparent,
                    borderRadius: BorderRadius.circular(8),
                  ),
                  child: Text('${pageIndex + 1}', style: TextStyle(
                    color: isCurrentPage ? Colors.white : Colors.white38,
                    fontWeight: isCurrentPage ? FontWeight.bold : FontWeight.normal, fontSize: 13,
                  )),
                ),
              ),
            );
          ...
          // Rows per page
          const Text('Rows:', style: TextStyle(color: Colors.white30, fontSize: 12)),
          const SizedBox(width: 6),
          Container(
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<int>(
                value: _rowsPerPage,
                items: [20, 50, 100].map((e) => DropdownMenuItem(value: e, child: Text('$e', style: const TextStyle(fontSize: 12)))).toList(),
                onChanged: (v) {
