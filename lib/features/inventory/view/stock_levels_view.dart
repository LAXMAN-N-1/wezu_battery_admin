...
final stockStationsProvider = FutureProvider.autoDispose<List<StationStock>>((ref) {
  final alertOnly = ref.watch(stockAlertFilterProvider);
  final sortBy = ref.watch(stockSortProvider);
  return ref.watch(stockRepositoryProvider).getStations(alertOnly: alertOnly, sortBy: sortBy);
});

final activeAlertsProvider = FutureProvider.autoDispose<List<StockAlert>>((ref) {
  return ref.watch(stockRepositoryProvider).getAlerts();
});

final stockViewModeProvider = StateProvider<String>((ref) => 'Grid'); // Grid, List, Map
final stockSortProvider = StateProvider<String>((ref) => 'utilization'); // utilization, available, name
final stockAlertFilterProvider = StateProvider<bool>((ref) => false); // false = all, true = low stock only
final _activeFilterProvider = StateProvider<String>((ref) => 'All Locations');

final stockLocationsProvider = FutureProvider.autoDispose<List<LocationStock>>((ref) {
...
class StockLevelsView extends ConsumerWidget {
          ...
          return InkWell(
            ...
            child: Container(
              ...
              child: Row(
                children: [
                  Icon(
                    mode == 'Grid' ? Icons.grid_view : (mode == 'List' ? Icons.list : Icons.map),
                    size: 16,
      ...
      data: (alerts) {
        ...
        return Column(
          children: alerts.map((alert) {
            ...
            final iconColor = isCritical ? Colors.redAccent : Colors.amberAccent;
            ...
            return ClipRRect(
              ...
              decoration: BoxDecoration(
                ...
                border: Border(
                  ...
                  right: BorderSide(color: bannerColor.withValues(alpha: 0.3)),
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    child: Icon(isCritical ? Icons.error_outline : Icons.warning_amber_rounded, color: iconColor, size: 24),
                  ).animate(onPlay: (c) => c.repeat()).fade(duration: 1000.ms, begin: 0.5, end: 1.0),
                        ...
                        Row(
                          children: [
                            Container(
                              ...
                              decoration: BoxDecoration(color: bannerColor, borderRadius: BorderRadius.circular(4)),
                  ...
                  TextButton.icon(
                    onPressed: () async {
                      ...
                      try {
                        ...
                        final detail = await repo.getStationDetail(alert.stationId);
                        if (context.mounted) {
                  ...
                  TextButton(
                    onPressed: () {
                      ref.read(stockRepositoryProvider).dismissAlert(alert.stationId, "Dismissed by admin");
    ...
    return overviewAsync.when(
      data: (overview) {
        return SingleChildScrollView(
          ...
          child: Row(
            children: [
              SizedBox(width: 280, child: _StatCard(title: 'Fleet Total', subTitle: 'Across ${overview.totalStations} stations + ${overview.warehouseCount > 0 || overview.serviceCount > 0 ? "2 locations" : "0 locations"}', value: '${overview.totalBatteries}', icon: Icons.battery_charging_full, color: Colors.blue)),
              const SizedBox(width: 16),
              SizedBox(width: 280, child: _StatCard(title: 'Available Now', subTitle: '${overview.totalBatteries > 0 ? (overview.availableCount / overview.totalBatteries * 100).toStringAsFixed(0) : 0}% of fleet ready to rent', value: '${overview.availableCount}', icon: Icons.check_circle_outline, color: Colors.green, onTap: () => _setSortAndFilter(ref, false, 'available'))),
              const SizedBox(width: 16),
              SizedBox(width: 280, child: _StatCard(title: 'In Active Rental', subTitle: 'Fleet utilization ${overview.avgUtilization.toStringAsFixed(0)}%', value: '${overview.rentedCount}', icon: Icons.directions_bike, color: Colors.purple)),
              const SizedBox(width: 16),
              SizedBox(width: 280, child: _StatCard(title: 'In Service/Maintenance', subTitle: '${overview.maintenanceCount} batteries need attention', value: '${overview.maintenanceCount}', icon: Icons.build, color: Colors.amber)),
              const SizedBox(width: 16),
              SizedBox(
                width: 280,
                child: _StatCard(
                  ...
                  color: overview.lowStockAlerts > 0 ? Colors.red : Colors.green,
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Row(
            children: [
              Text('Sort by:', style: TextStyle(color: Colors.white.withValues(alpha: 0.5), fontSize: 13)),
              const SizedBox(width: 8),
              DropdownButtonHideUnderline(
                child: DropdownButton<String>(
                  ...
                  icon: Icon(Icons.keyboard_arrow_down, color: Colors.white.withValues(alpha: 0.5), size: 16),
                  items: const [
                    DropdownMenuItem(value: 'utilization', child: Text('Highest Utilization')),
                    DropdownMenuItem(value: 'available', child: Text('Lowest Available')),
                    DropdownMenuItem(value: 'name', child: Text('Alphabetical')),
                  ],
                  onChanged: (val) {
                    if (val != null) ref.read(stockSortProvider.notifier).state = val;
                  },
                ),
              ),
  ...
  Widget _buildFilterPill(WidgetRef ref, String label, String activeFilter, bool isAlert) {
    ...
    return FilterChip(
      ...
      labelStyle: TextStyle(color: isSelected ? baseColor : Colors.white.withValues(alpha: 0.7)),
      side: BorderSide(color: isSelected ? baseColor.withValues(alpha: 0.5) : Colors.white.withValues(alpha: 0.1)),
    );
  }
      ...
      data: (stations) {
        return locationsAsync.when(
          data: (locations) {
            ...
            } else if (activeFilter == 'Stations Only' || activeFilter == 'Low Stock') {
              items.addAll(stations);
            } else if (activeFilter == 'Critical (<10%)') {
              items.addAll(stations.where((s) => s.utilizationPercentage < 10));
            } else if (activeFilter == 'Warehouse') {
              items.addAll(locations.where((l) => l.locationType == 'WAREHOUSE'));
            } else if (activeFilter == 'Service Center') {
              items.addAll(locations.where((l) => l.locationType == 'SERVICE_CENTER'));
            }
          ...
          error: (err, _) => SliverFillRemaining(child: Center(child: Text('Error loading locations: $err', style: const TextStyle(color: Colors.red)))),
        );
      },
      loading: () => const SliverFillRemaining(child: Center(child: CircularProgressIndicator())),
  ...
  Widget _buildListView(List<dynamic> items, {required BuildContext context}) {
    return Container(
      ...
      child: DataTable(
        ...
        dataRowMinHeight: 65,
        columns: const [
          DataColumn(label: Text('Station', style: TextStyle(color: Colors.white54, fontSize: 12))),
          DataColumn(label: Text('Available', style: TextStyle(color: Colors.white54, fontSize: 12))),
          DataColumn(label: Text('Rented', style: TextStyle(color: Colors.white54, fontSize: 12))),
          DataColumn(label: Text('Service', style: TextStyle(color: Colors.white54, fontSize: 12))),
          DataColumn(label: Text('Utilization', style: TextStyle(color: Colors.white54, fontSize: 12))),
          DataColumn(label: Text('Health', style: TextStyle(color: Colors.white54, fontSize: 12))),
          ...
          if (item is StationStock) {
            ...
            return DataRow(
              cells: [
                ...
                DataCell(
                  Text(
                    '${s.availableCount}', 
                    style: TextStyle(color: isCritical ? Colors.redAccent : Colors.white, fontWeight: isCritical ? FontWeight.bold : FontWeight.normal)
                  )
                ),
                DataCell(Text('${s.rentedCount}', style: TextStyle(color: Colors.white.withValues(alpha: 0.7)))),
                DataCell(Text('${s.maintenanceCount}', style: TextStyle(color: Colors.white.withValues(alpha: 0.5)))),
                DataCell(
                  Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                ...
                DataCell(
                  TextButton(
                    onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => StationStockDetailView(stationId: s.stationId))),
            ...
            return DataRow(
              cells: [
                ...
                DataCell(Text('${l.availableCount}', style: const TextStyle(color: Colors.white))),
                DataCell(Text('-', style: TextStyle(color: Colors.white.withValues(alpha: 0.3)))),
                DataCell(Text('${l.maintenanceCount}', style: TextStyle(color: Colors.white.withValues(alpha: 0.5)))),
                DataCell(Text('-', style: TextStyle(color: Colors.white.withValues(alpha: 0.3)))),
                DataCell(
                   Container(
                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
...
class _StatCard extends StatelessWidget {
  ...
  const _StatCard({required this.title, required this.subTitle, required this.value, required this.icon, required this.color, this.isAlert = false, this.onTap});
      ...
      child: ClipRRect(
        ...
        child: Container(
          ...
          children: [
              ...
              children: [
                ...
                if (isAlert)
                  Container(
                    ...
                    child: const Text('⚠ ACTION REQ', style: TextStyle(color: Colors.redAccent, fontSize: 10, fontWeight: FontWeight.bold)),
            ...
            Text(
              subTitle,
              style: TextStyle(fontSize: 12, color: Colors.white.withValues(alpha: 0.5), fontWeight: FontWeight.w400),
            ),
          ],
        ),
        ),
...
class _StationStockCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final statusColor = isCritical ? Colors.red : (isWarning ? Colors.amber : Colors.green);
        ...
        children: [
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      const Text('ACTIVE', style: TextStyle(color: Colors.white70, fontSize: 9, fontWeight: FontWeight.bold, letterSpacing: 0.5)),
                    ],
                  ),
                ),
              ],
          ...
          Padding(
            ...
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                _buildMiniStat('Available', '${station.availableCount}', isCritical ? Colors.redAccent : Colors.white),
                _buildMiniStat('Rented', '${station.rentedCount}', Colors.white70),
                _buildMiniStat('Service', '${station.maintenanceCount}', Colors.amber.withValues(alpha: 0.8)),
              ],
            ),
          ),
            ...
            child: Column(
              ...
              children: [
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text('${station.utilizationPercentage.toInt()}% utilized', style: TextStyle(color: Colors.white.withValues(alpha: 0.9), fontSize: 13, fontWeight: FontWeight.w600)),
                    Text('${station.totalAssigned} total batteries', style: TextStyle(color: Colors.white.withValues(alpha: 0.5), fontSize: 12)),
                  ],
                ),
                const SizedBox(height: 8),
                ClipRRect(
                  ...
                  child: Row(
                    children: [
                      ...
                      if (station.config != null && station.config!.maxCapacity > station.totalAssigned)
                        Expanded(
                          flex: station.config!.maxCapacity - station.totalAssigned,
                          child: Container(height: 8, color: const Color(0xFF0F172A)),
                        ),
                    ],
                  ),
                ),
          ...
          Padding(
            ...
            child: Row(
              children: [
                Icon(Icons.inventory_2_outlined, size: 14, color: Colors.white.withValues(alpha: 0.4)),
                const SizedBox(width: 6),
                Text('Reorder Point: ${station.config?.reorderPoint ?? "N/A"}', style: TextStyle(fontSize: 12, color: Colors.white.withValues(alpha: 0.5))),
                const Spacer(),
                Icon(Icons.access_time, size: 14, color: Colors.white.withValues(alpha: 0.4)),
                const SizedBox(width: 4),
                Text('Just now', style: TextStyle(fontSize: 11, color: Colors.white.withValues(alpha: 0.4))),
              ],
            ),
          ),
          ...
          Row(
            children: [
              Expanded(
                child: TextButton(
                  onPressed: () {
                     Navigator.push(context, MaterialPageRoute(builder: (_) => StationStockDetailView(stationId: station.stationId)));
                  },
                ...
                child: Consumer(
                  builder: (context, ref, _) => TextButton(
                    onPressed: () async {
                      ...
                      try {
                        final repo = ref.read(stockRepositoryProvider);
                        final detail = await repo.getStationDetail(station.stationId);
                        if (context.mounted) {
                          Navigator.pop(context); // Close loading
                          showDialog(
                            context: context,
                         ...
                         if (context.mounted) {
                           Navigator.pop(context); // Close loading
                           ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error loading forecast: $e'), backgroundColor: Colors.red));
                         }
                      }
                    },
                    style: TextButton.styleFrom(
...
class _LocationStockCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final statusColor = isWarehouse ? const Color(0xFF3B82F6) : const Color(0xFFA855F7); // Blue for Warehouse, Purple for Service
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 4),
                      Text(
                        isWarehouse ? 'Central Storage Facility' : 'Maintenance & Repair',
                        style: TextStyle(color: Colors.white.withValues(alpha: 0.5), fontSize: 12),
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                      ),
              ...
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      _buildLargeStat('Available\nInventory', '${location.availableCount}', Colors.white),
  ...
  Widget _buildLargeStat(String label, String value, Color color) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        Text(label, textAlign: TextAlign.center, style: TextStyle(color: Colors.white.withValues(alpha: 0.5), fontSize: 12, fontWeight: FontWeight.w500)),
      ],
    );
  }
}
