...
class _DealersViewState extends State<DealersView> {
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 16),
              _buildStatCard('All-time Commissions', '₹${NumberFormat('#,##0.00').format(_stats?.totalCommissionsPaid ?? 0)}', Icons.payments_outlined, const Color(0xFF22C55E)),
            ],
              ...
              Container(
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                    ...
                    items: [
                      const DropdownMenuItem(value: 'all', child: Text('All Cities', style: TextStyle(color: Colors.white))),
                      ...cities.map((c) => DropdownMenuItem(value: c, child: Text(c, style: const TextStyle(color: Colors.white)))),
                    ],
                    onChanged: (v) { _cityFilter = v ?? 'all'; _loadData(); },
                  ),
                ),
              ),
            ],
          ...
          AdvancedCard(
            ...
            child: _isLoading
              ...
              : _dealers.isEmpty
                ? const SizedBox(height: 300, child: Center(child: Text('No dealers found.', style: TextStyle(color: Colors.white54))))
                : AdvancedTable(
                    columns: const ['Business Name', 'City', 'Contact', 'GST / PAN', 'Status', 'Joined', 'Actions'],
                    rows: _dealers.map((d) {
                      return [
                        Row(
                          children: [
                            Container(
                              ...
                              decoration: BoxDecoration(
                                gradient: LinearGradient(colors: [const Color(0xFF3B82F6).withValues(alpha: 0.3), const Color(0xFF8B5CF6).withValues(alpha: 0.3)]),
                                borderRadius: BorderRadius.circular(10),
                              ),
                              child: Center(child: Text(d.businessName[0], style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold))),
                            ),
                            const SizedBox(width: 12),
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(d.contactEmail ?? '', style: const TextStyle(color: Colors.white38, fontSize: 11)),
                                ],
                              ),
                            ),
                          ],
                        ...
                        Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(d.gstNumber ?? 'N/A', style: const TextStyle(color: Colors.white70, fontSize: 12)),
                            Text(d.panNumber ?? 'N/A', style: const TextStyle(color: Colors.white38, fontSize: 11)),
                          ],
                        ),
                        StatusBadge(status: d.isActive ? 'Active' : 'Inactive'),
                        Text(DateFormat('MMM dd, yyyy').format(d.createdAt), style: const TextStyle(color: Colors.white54, fontSize: 12)),
                        Row(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Container(
                      ...
                      child: Center(child: Text(d.businessName[0], style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 20))),
                    ),
                    const SizedBox(width: 16),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(d.businessName, style: GoogleFonts.outfit(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.white)),
                          StatusBadge(status: d.isActive ? 'Active' : 'Inactive'),
                        ],
                      ),
                    ),
                ...
                _detailSection('Contact Info', [
                  _detailRow(Icons.person_outline, 'Contact Person', d.contactPerson),
                  _detailRow(Icons.email_outlined, 'Email', d.contactEmail ?? 'N/A'),
                  _detailRow(Icons.phone_outlined, 'Phone', d.contactPhone),
                ]),
                _detailSection('Location', [
                  _detailRow(Icons.home_outlined, 'Address', d.addressLine1 ?? 'N/A'),
                ...
                _detailSection('Compliance', [
                  _detailRow(Icons.receipt_long_outlined, 'GST Number', d.gstNumber ?? 'N/A'),
                  _detailRow(Icons.credit_card, 'PAN Number', d.panNumber ?? 'N/A'),
                ]),
                _detailSection('Other', [
                  _detailRow(Icons.calendar_today_outlined, 'Joined', DateFormat('MMM dd, yyyy').format(d.createdAt)),
                ]),
              ],
            ),
          ),
  ...
  Widget _detailSection(String title, List<Widget> children) {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(title.toUpperCase(), style: GoogleFonts.inter(fontSize: 10, color: Colors.white38, letterSpacing: 1.5, fontWeight: FontWeight.bold)),
  ...
  void _showDealerFormDialog(DealerProfile? dealer) {
    ...
    final contactCtrl = TextEditingController(text: dealer?.contactPerson ?? '');
        ...
        child: Container(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(isEdit ? 'Edit Dealer' : 'Add New Dealer', style: GoogleFonts.outfit(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.white)),
                const SizedBox(height: 8),
                Text(isEdit ? 'Update dealer profile information.' : 'Register a new dealer partner.', style: const TextStyle(color: Colors.white54)),
                const SizedBox(height: 24),
                Row(children: [
                  Expanded(child: _formField('Business Name *', bizCtrl)),
                  const SizedBox(width: 16),
                  Expanded(child: _formField('Contact Person *', contactCtrl)),
                ]),
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () async {
                        ...
                        bool success;
                        if (isEdit) {
                          success = await _repository.updateDealer(dealer.id, data);
                        } else {
                        ...
                        if (success && mounted) {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                            content: Text(isEdit ? 'Dealer updated!' : 'Dealer created!'),
