...
class _EditUserDialogState extends ConsumerState<EditUserDialog> {
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Container(
                    ...
                    child: const Icon(Icons.edit, color: Colors.amber, size: 22),
                  ),
                  const SizedBox(width: 14),
                  Text('Edit User Profile', style: GoogleFonts.outfit(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
                  const Spacer(),
              ...
              if (hasRoleChanged) ...[
                const SizedBox(height: 16),
                _buildField('Reason for Role Change', _roleReasonController, Icons.info_outline),
              ],
  ...
  void _submit() async {
    ...
    if (hasRoleChanged && _roleReasonController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please provide a reason for role change')));
      return;
    }
    ...
    try {
      if (hasRoleChanged) {
        final role = _availableRoles.firstWhere((r) => r.name.toLowerCase() == _selectedRole.toLowerCase());
        await ref.read(userListProvider.notifier).changeUserRole(
          widget.user.id, 
          roleId: role.id, 
