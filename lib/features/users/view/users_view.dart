...
class _UsersViewState extends ConsumerState<UsersView>
                ...
                Row(
                  children: [
                    ...
                    Expanded(
                      child: _buildStatCard(
                        'Active',
                        userState.activeUsers.toString(),
                ...
                Row(
                  children: [
                    Expanded(
                      child: _buildStatCard(
                        'Suspended',
                        userState.suspendedUsers.toString(),
                        Icons.block,
      ...
      children: [
        ...
        if (mobile)
          TextField(
            ...
            onChanged: (v) =>
                ref.read(userListProvider.notifier).setSearchQuery(v),
          ...
          Row(
            children: [
              ...
              _buildFilterDropdown(
                ...
                onChanged: (v) =>
                    ref.read(userListProvider.notifier).setRoleFilter(v),
              ),
              const SizedBox(width: 12),
              _buildFilterDropdown(
                ...
                onChanged: (v) =>
                    ref.read(userListProvider.notifier).setStatusFilter(v),
              ),
            ],
          ).animate().fadeIn(duration: 400.ms, delay: 150.ms),
        const SizedBox(height: 16),
        ...
        Expanded(
          child: state.isLoading
              ? const Padding(
                  ...
                  child: WezuSkeletonTable(rows: 8, columns: 6),
                )
              : mobile
            ...
            child: Row(
              ...
              children: [
                ...
                IconButton(
                  ...
                  onPressed:
                      state.page < (state.totalCount / state.limit).ceil()
                      ? () => ref
                            .read(userListProvider.notifier)
                            .goToPage(state.page + 1)
                      : null,
  ...
  Widget _buildInvitesTab(InviteListState state, bool mobile) {
    if (state.isLoading) {
      return const Padding(
        padding: EdgeInsets.all(12),
        child: WezuSkeletonTable(rows: 8, columns: 3),
      );
    }
    return Column(
      children: [
    ...
    return FutureBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (ctx, snap) {
        if (snap.connectionState != ConnectionState.done) {
          return const Padding(
            padding: EdgeInsets.all(12),
            child: WezuSkeletonTable(rows: 5, columns: 4),
          );
        }
        if (snap.hasError) {
          return AdvancedCard(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (_, i) {
              ...
              return ListTile(
                    ...
                    trailing: Text(
                      DateFormat('MMM d').format(e['date'] as DateTime),
                      style: const TextStyle(
                        color: Colors.white24,
                        fontSize: 11,
                      ),
  ...
  void _showInviteDialog() => showDialog(
    context: context,
    builder: (_) => InviteUsersDialog(
      onSingleInvite: (email, role) async {
        await ref
            .read(inviteListProvider.notifier)
            .sendInvite(email: email, role: role);
          ...
          builder: (_) => SuspendUserDialog(
            ...
            onSubmit: (reason) async {
              try {
                await ref
                    .read(userListProvider.notifier)
                    .suspendUser(user.id, reason: reason);
