...
class AnalyticsRepository {
  ...
  Future<List<FraudRisk>> getFraudRisks() async {
    ...
    return items.whereType<Map>().map((raw) {
      ...
      return FraudRisk(
        userId: userId,
        userName:
            userNames[userId] ?? item['email']?.toString() ?? 'User #$userId',
        score: score,
  ...
  Future<List<InviteLink>> getInviteLinks() async {
    ...
    return items.whereType<Map>().map((raw) {
      ...
      return InviteLink(
        ...
        isUsed:
            item['is_used'] == true ||
            (item['status']?.toString().toLowerCase() == 'accepted'),
      );
    }).toList();
  }
  ...
  Future<List<Map<String, dynamic>>> getLoginHistory() async {
    ...
    return entries
        .map(
          (entry) => {'date': DateTime.parse(entry.key), 'logins': entry.value},
        )
        .toList();
  }
  ...
  Future<List<Map<String, dynamic>>> getRentalFrequency() async {
    ...
    return recent.map((entry) {
      ...
      return {'month': monthLabels[month], 'rentals': entry.value};
    }).toList();
  }
